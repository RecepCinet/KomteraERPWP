...
from " . getTableName('aa_erp_kt_acronis_fatura_kes') . " temp
left join " . getTableName('aa_erp_kt_Acronis_CH_Eslesme') . " e ON e.acronis_bayi_adi = temp.acronis_bayi
left join " . getTableName('aaa_erp_kt_bayiler') . " b ON e.komtera_bayi_adi  = b.CH_UNVANI
left join " . getTableName('aa_erp_kt_fiyat_listesi') . " f ON f.sku  = temp.sku
...
for ($t=0;$t<count($data);$t++) {
    ...
    $sqlinsert = "INSERT INTO LKS.dbo." . getTableName('ARYD_FIS_AKTARIM') . " ([SIPARISID],[NO],[CARIKOD],[MALZEMEKOD],[MIKTAR],[FIYAT],[SATIS_TEMSILCISI],[SERI_NO]
...
$sqlupdate = "update LKS.dbo." . getTableName('ARYD_FIS_AKTARIM') . " set SONUC='0' WHERE SONUC='8'";
try {
