...
try {
    ...
    if (!$file_info) {
        throw new Exception('Lisans dosyası bulunamadı');
    }
    ...
    $physical_file_path = dirname(__DIR__) . '/uploads/lisans_dosyalar/' . $file_info['FILE_PATH'];
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Lisans dosyası başarıyla silindi'
    ]);
    ...
    error_log('Delete Lisans File Error Details: ' . json_encode($error_details));
    ...
    error_log('Delete Lisans File Fatal Error Details: ' . json_encode($fatal_details));
    ...
    error_log('Delete Lisans File Throwable Error: ' . json_encode($throwable_details));
