...
stuff((
	...
	from aa_erp_kt_siparisler where X_FIRSAT_NO=f.FIRSAT_NO
...
WHEN PARA_BIRIMI = 'TRY' THEN (select SUM(tu.ADET*tu.B_SATIS_FIYATI) from aa_erp_kt_teklifler_urunler tu INNER JOIN aa_erp_kt_teklifler te  ON tu.X_TEKLIF_NO = te.TEKLIF_NO  where te.PDF=1 AND te.TEKLIF_TIPI=1 AND te.X_FIRSAT_NO=f.FIRSAT_NO)/CONVERT(money," . $_SESSION['USD'] . ")
WHEN PARA_BIRIMI = 'EUR' THEN (select SUM(tu.ADET*tu.B_SATIS_FIYATI) from aa_erp_kt_teklifler_urunler tu INNER JOIN aa_erp_kt_teklifler te  ON tu.X_TEKLIF_NO = te.TEKLIF_NO  where te.PDF=1 AND te.TEKLIF_TIPI=1 AND te.X_FIRSAT_NO=f.FIRSAT_NO)*CONVERT(money," . $_SESSION['EUR'] . "/" . $_SESSION['USD'] . ")
ELSE 0
END AS DLR_TUTAR,
f.REGISTER,
f.BASLANGIC_TARIHI,
...
FROM LKS.dbo.aa_erp_kt_firsatlar f WHERE SIL='0' AND f.MUSTERI_TEMSILCISI='" . $_SESSION['user']['kullanici'] . "'
";
