...
for xml path (''), type).value('.','nvarchar(max)'),1,1,'') as skular,
    ...
    LEFT JOIN " . getTableName('aa_erp_kt_teklifler') . " t ON tu.X_TEKLIF_NO = t.TEKLIF_NO
    LEFT JOIN " . getTableName('aa_erp_kt_fiyat_listesi') . " fl ON tu.SKU = fl.SKU
...
FROM LKS.dbo." . getTableName('aa_erp_kt_firsatlar') . " f WHERE DURUM=0 AND SIL='0'
