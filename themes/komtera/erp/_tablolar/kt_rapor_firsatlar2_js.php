...
<script>
    ...
    var gridPageName = 'kt_rapor_firsatlar2_js';
...
//            },
            ...
            {title: "<?php echo __('quotations','komtera'); ?>",exportRender: false, style: {'text-color': '#dd0000'}, dataIndx: "Teklifler", align: "left", editable: false, minWidth: 90, sortable: false,
                render: function (ui) {
                    var out = "";
            ...
            },{title: "<?php echo __('skus','komtera'); ?>",filter: {
    crules: [{condition: 'contain'}]
    }, editable: false, minWidth: 90, sortable: true, dataIndx: "skular"},
            {title: "<?php echo __('orders','komtera'); ?>",exportRender: false, style: {'text-color': '#dd0000'}, dataIndx: "Siparisler", align: "left", editable: false, minWidth: 90, sortable: false,
                render: function (ui) {
                    var out = "";
                    var data = ui.rowData.Siparisler;
                    if (data) {
                        var tek = data.split(',');
                        for (let i = 0; i < tek.length; i++) {
                            if (i > 0) {
                            ...
                            out = out + "<a href='#' class='acac' onclick='SiparisAc(\"" + tek[i] + "\")'>" + tek[i] + "</a>";
                            //out = out + "<a href='#' class='demo_ac' onclick='TeklifAc(\"" + tek[i] + "\")'><span class='ui-icon ui-icon-zoomin'></span></a>" + tek[i];
                        }
                        return out;
                    }
            ...
            {title: "<?php echo __('last_change','komtera'); ?>", minWidth: 80, dataIndx: "REVIZE_TARIHI", dataType: "date", format: 'dd.mm.yy'},
            {title: "<?php echo __('brand','komtera'); ?>", hidden: false, editable: false, minWidth: 110, sortable: true, dataIndx: "MARKA", filter: {
                    crules: [{condition: 'range'}]
                }
            },
			{title: "<?php echo __('brand_manager','komtera'); ?>", hidden: false, editable: false, minWidth: 110, sortable: true, dataIndx: "MARKA_MANAGER", filter: {
                    crules: [{condition: 'range'}]
                }
            },
            ...
            {title: "<?php echo __('created_by','komtera'); ?>",
...
//            },
            {title: "<?php echo __('amount','komtera'); ?>", dataType: "float", render: function (ui) {
                    if (ui.cellData === null) {
            ...
            {title: "USD",dataType: "float",render: function (ui) {
                    if (ui.cellData === null) {
                        return {style: {"background": "#FF8888"}};
                    }
                },
				summary: {type: "sum", edit: true}, align: "right", format: "#.###,00", exportRender: true,
				editable: false, minWidth: 90, sortable: true, dataIndx: "DLR_TUTAR"},
            {title: "<?php echo __('dealer','komtera'); ?>", editable: false, minWidth: 220, sortable: true, dataIndx: "BAYI_ADI", filter: {
                    crules: [{condition: 'contain'}]
                }
            },
			{title: "<?php echo __('dealer_contact','komtera'); ?>", hidden: false, editable: false, minWidth: 110, sortable: true, dataIndx: "BAYI_YETKILI_ISIM", filter: {
                    crules: [{condition: 'range'}]
                }
            },
            ...
            {title: "<?php echo __('probability','komtera'); ?>", minWidth: 210, dataIndx: "OLASILIK",
                render: function (ui) {
			 ...
			 {title: "<?php echo __('probability_2','komtera'); ?>", minWidth: 210, dataIndx: "OLASILIK",
             
                filter: {
                    crules: [{condition: 'contain'}]
                }
            },
            {title: "<?php echo __('loss_reason','komtera'); ?>", editable: false, minWidth: 160, sortable: true, dataIndx: "KAYBEDILME_NEDENI", filter: {
                    crules: [{condition: 'contain'}]
                }
            },
            {title: "<?php echo __('loss_reason_other','komtera'); ?>", editable: false, minWidth: 150, sortable: true, dataIndx: "KAYBEDILME_NEDENI_DIGER", filter: {
                    crules: [{condition: 'contain'}]
                }
            },{title: "<?php echo __('activity','komtera'); ?>", editable: false, minWidth: 150, sortable: true, dataIndx: "ETKINLIK", filter: {
                    crules: [{condition: 'contain'}]
                }
            },
                {title: "<?php echo __('notes','komtera'); ?>", editable: false, minWidth: 150, sortable: true, dataIndx: "TNOTLAR", filter: {
                    crules: [{condition: 'contain'}]
                }
                }
        ];
        var dataModelSS = {
            ...
            method: "GET",
            recIndx: "id",
            url: "_tablolar/kt_rapor_firsatlar2.php?dbname=LKS&date1=<?PHP echo $date1; ?>&date2=<?PHP echo $date2; ?>",
...
//                type: 'local',
        ...
        grid = pq.grid("div#grid_rapor_firsatlar2", obj);
