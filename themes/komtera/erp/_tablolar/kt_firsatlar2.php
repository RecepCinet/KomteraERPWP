...
stuff((
select ','+convert(varchar(50),KAMPANYA)
from aa_erp_kt_teklifler where X_FIRSAT_NO=f.FIRSAT_NO
for xml path (''), type).value('.','nvarchar(max)'),1,1,'') as KAMPANYALAR,
stuff((
...
FROM LKS.dbo.aa_erp_kt_firsatlar f WHERE SIL='0' AND CHARINDEX(MARKA, '" . implode(',', get_user_meta(get_current_user_id(), 'my_brands', true) ?: []) . "')>0 and $dates
