...
if (isset($_GET["pq_curpage"]) && isset($_GET["pq_rpp"])) {
    ...
    $sql = "Select count(*) from aa_erp_kt_musteriler $filterQuery";
...
//    $total_Records = $stmt->fetchColumn();
    ...
    $sql = "Select * from aa_erp_kt_musteriler $filterQuery $sortQuery OFFSET $skip ROWS FETCH NEXT $pq_rPP ROWS ONLY";
    $stmt = $conn->prepare($sql);
    $stmt->execute($filterParam);
