...
foreach ($su as $key => $satir) {
    ...
    if ($malzemeTip == 1) {
            ...
            $urunler[] = array("x_siparisNO" => (string) $_NO,
            ...
            $hizmetler[] = array("x_siparisNO" => (string) $_NO,
                "kod" => (string) $_MALZEMEKOD,
                //"malzemeTip" => (string) $malzemeTip,
                "birim" => $_BIRIM,
                "miktar" => (int) $_MIKTAR,
...
$gitdata=json_encode($fatura);
trace($gitdata);

// Eger Daha once Irsaliyesi basilmissa bi daha gonderme ve insert etmesin diye eklendi!
if (count($fat)==0) {
...
//FLOW Paket Gidiyor Irsaliye
$sonuc = file_get_contents("http://172.16.85.107:$port/api/Dispatch/Sales/Insert", false, $context);
BotMesaj(json_encode($gitdata,JSON_HEX_TAG | JSON_HEX_APOS | JSON_UNESCAPED_UNICODE) . "\n\n" . $sonuc);
echo $sonuc;
$arr= json_decode($sonuc,true);
$hata="";
trace($arr);

$hataham=print_r($fatura,true);
unset($fatura["products"]);

$fatura['belgeTarihi']=$fatura['irsaliyeTarihi'];
unset($fatura['irsaliyeTarihi']);

trace($fatura);

$res=json_decode($arr['response'],true);
if ($arr['result']=="error") {
    $hata=($res['ModelState']['ValError0'][0]);
    echo $hata;
    if ($hata=="") {
        $hata = print_r($arr['response'], true);
    }
    $hatagonder = "Gonderilen: \n" . $hataham . "\n\nGelen Cevap:\n" . $hata;
    MailGonder("order@komtera.com", "$siparis_no nolu sipariste irsaliye/fatura olusturmada hata!",  $hatagonder);
    //MailGonder("recep@piksel.biz", "$siparis_no nolu sipariste irsaliye/fatura olusturmada hata!",  $hatagonder);
    die();
} else {
    $fisID=$res['LogoId'];
    $fisNO=$res['FisNo'];
...
try {
    ...
    $sqlinsert = "INSERT INTO aa_erp_kt_fatura $str";
...
foreach ($urunler as $satir) {
    try {
        ...
        $sqlinsert = "INSERT INTO aa_erp_kt_fatura_urunler $str";
        trace($sqlinsert);
        $stmt = $conn->prepare($sqlinsert);
...
// FLOW Fatura Bastir
// Butun Irsaliyeleri Bas!
if ($fis_dur == "1") {
    $url = "select LogoId
from aa_erp_kt_fatura f
where f.teklifNO =:teklif_no
group by LogoId";
    $stmt = $conn->prepare($url);
    $stmt->execute(['teklif_no' => $teklif_no]);
    $t = $stmt->fetchAll(PDO::FETCH_COLUMN);
    $token = Token();
    if ($_SevkiyatKime=="Bayi") {
        $fat_adres="";
    }
    $ham = Array(
        "faturaTarihi" => $baskiTarihi,
        "siparisNo" => $teklif_no,
        "musteriSiparisNo" => (string) $MusteriSiparisNo,
        "adres" => $_Sevk_Adresi,
        "unvan" => mb_substr(trim((string) $BayiMusteri),0,50),
        "kisiBilgi" => (string) $KisiBilgi,
        "irsaliyeIdListesi" => $t
    );
    $ham["services"]=$hizmetler;
    print_r($ham);
    $options = array('http' => array(
            'ignore_errors' => true,
            'method' => 'POST',
            'header' => "Authorization: Bearer $token\r\n" .
            "Content-Type: application/json",
            'content' => json_encode($ham)
    ));
    $context = stream_context_create($options);
    ini_set('default_socket_timeout', 600);
    $sonuc = file_get_contents("http://172.16.85.107:$port/api/Dispatch/DispatchBilling", false, $context);
    $arr = json_decode($sonuc, true);
    $result = $arr['result'];
    $response = json_decode($arr['response'], true);
    print_r($arr);
    print_r($response);
    BotMesaj(json_encode($ham,JSON_HEX_TAG | JSON_HEX_APOS | JSON_UNESCAPED_UNICODE) . "\n\n" . $sonuc);
}
?>
