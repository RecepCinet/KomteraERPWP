...
if (isset($_GET["pq_curpage"]) && isset($_GET["pq_rpp"])) {
    ...
    $sql = "Select count(*) from aa_erp_kt_fiyat_listesi $filterQuery";
...
//    $total_Records = $stmt->fetchColumn();
    ...
    $sql = "Select * from aa_erp_kt_fiyat_listesi $filterQuery $sortQuery OFFSET $skip ROWS FETCH NEXT $pq_rPP ROWS ONLY";
    $stmt = $conn->prepare($sql);
    $stmt->execute($filterParam);
